...
#define LINE_TRACKER_TEST_BASE_SPEED_MM_S (380.0F)
#define LINE_TRACKER_TEST_PID_KP          (43.0F)
    ...
    line_tracker_config_struct *config)
{
    ...
    config->track_all_active_as_center = 0U;
    ...
    line_tracker_config_struct *config)
{
    ...
    config->base_speed_mm_s = LINE_TRACKER_TEST_BASE_SPEED_MM_S;
    config->pid_kp = LINE_TRACKER_TEST_PID_KP;
...
static uint32 line_tracker_test_self_check(void)
{
    ...
    for (mask = 0U; mask <= 0xFFU; mask++)
    {
        ...
        {
            expected_correction = sensor.deviation
                * LINE_TRACKER_TEST_PID_KP;
            ...
            expected_left = line_tracker_test_clamp_target(
                LINE_TRACKER_TEST_BASE_SPEED_MM_S + expected_correction);
            expected_right = line_tracker_test_clamp_target(
                LINE_TRACKER_TEST_BASE_SPEED_MM_S - expected_correction);
            pid_error = status.pid_error - sensor.deviation;
            if (pid_error < 0.0F)
            {
                pid_error = -pid_error;
            }
            ...
            if ((status.state != LINE_TRACKER_STATE_TRACKING)
                ...
                || (output.left_target_mm_s < 0.0F)
                || (output.right_target_mm_s < 0.0F)
                || (pid_error > LINE_TRACKER_TEST_FLOAT_EPSILON)
                || (line_tracker_test_float_is_near(
                        status.correction_mm_s,
...
void test_line_tracker_run(void)
{
    ...
    ili9341_show_string(8U, 232U, "PIDERR :");
