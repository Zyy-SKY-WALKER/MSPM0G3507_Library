...
// 接入dl1b激光测距模块 对应主板 TOF 接口 请注意线序
//      模块管脚            单片机管脚
//      SCL                 查看 zf_device_dl1b.h 中 dl1b_SCL_PIN  宏定义的引脚
//      SDA                 查看 zf_device_dl1b.h 中 dl1b_SDA_PIN  宏定义的引脚
//      VCC                 5V 电源
//      GND                 电源地

// *************************** 例程测试说明 ***************************
// 1.核心板烧录完成本例程 将核心板插在主板上 插到底 dl1b接在主板的 TOF 接口 注意线序
// 2.主板上电 或者核心板链接完毕后上电 再将调试下载器或者 USB-TTL 模块连接电脑
// 3.电脑上使用串口调试助手打开对应的串口 串口波特率为 zf_common_debug.h 文件中 DEBUG_UART_BAUDRATE 宏定义 默认 115200，核心板按下复位按键
// 4.观察核心板上是否有蓝灯缓慢闪烁，若闪烁代表初始化失败 请检查线序
// 5.核心板上蓝灯未闪烁，上位机将显示实时的距离数据
...
//      如果串口和接线都正常 查看LED灯是否闪烁 如果闪烁证明初始化失败 检查模块接线是否正常 线序是否正确
//      如果模块接线都正常 LED灯不闪烁 联系技术客服即可
// 问题2：有数据但是数据不正确
