...
int main(void)
{
	...
	pwm_init(MOTOR1_PWM1, 17000, 0);											        // PWM初始化频率 17KHz 占空比初始为 0
	pwm_init(MOTOR1_PWM2, 17000, 0);											        // PWM初始化频率 17KHz 占空比初始为 0
    ...
    while(1)
    {
        if(duty >= 0)                                                           // 左侧正转
        {
            pwm_set_duty(MOTOR1_PWM1, duty * (PWM_DUTY_MAX / 100));                  // 计算占空比
            pwm_set_duty(MOTOR1_PWM2, 0);                                            // 同一时间 一个电机只能输出一个 PWM 另一通道保持低电平
        ...
        {
            pwm_set_duty(MOTOR1_PWM1, 0);                                            // 同一时间 一个电机只能输出一个 PWM 另一通道保持低电平
            pwm_set_duty(MOTOR1_PWM2, (-duty) * (PWM_DUTY_MAX / 100));               // 计算占空比
