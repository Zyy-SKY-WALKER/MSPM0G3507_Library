...
// *************************** 例程测试说明 ***************************
// 1.该例程为IPS200PRO显示日历控件控制示例
// 2.核心板烧录本例程 插在主板上
//   2寸IPSPRO 显示模块插在主板的屏幕接口排座上
//   请注意引脚对应 不要插错
// 2.电池供电 上电后 2寸IPS 屏幕亮起 并且底部显示“Calendar测试”
// 3.主页面将显示一个日历控件，并且展示日历控件的不同展示效果
...
int main(void)
{
    ...
    // 初始化屏幕并创建一个页面
    page_id[0] = ips200pro_init("Calendar测试", IPS200PRO_TITLE_BOTTOM, 30);
    ...
    // 创建一个指针类型的时钟 直径200个像素
    calendar_id = ips200pro_calendar_create(20, 0, 200, 240);

    // 设置时钟的各种颜色属性
    ips200pro_set_color(calendar_id, COLOR_FOREGROUND, RGB565_WHITE);
    ips200pro_set_color(calendar_id, COLOR_BACKGROUND, IPS200PRO_RGB888_TO_RGB565(0x43, 0x4A, 0x61));
    ips200pro_set_color(calendar_id, COLOR_BORDER, RGB565_WHITE);
    ips200pro_set_color(calendar_id, COLOR_CALENDAR_YEAR, RGB565_WHITE);
    ips200pro_set_color(calendar_id, COLOR_CALENDAR_WEEK, RGB565_WHITE);
    ips200pro_set_color(calendar_id, COLOR_CALENDAR_TODAY, IPS200PRO_RGB888_TO_RGB565(0xFF, 0x71, 0x10));


    // 设置系统时间
    ips200pro_set_date(2025, 2, 17);
    // 获取系统的日期
    ips200pro_get_date(&ips200pro_time);
    ...
    while(true)
    {
        ...
        // 显示2030年2月的日历，可用于翻看日历，2：设置日历为英文模式
        ips200pro_calendar_display(2030, 2, IPS200PRO_CALENDAR_ENGLISH);


        system_delay_ms(2000);
        // 设置日历显示月份，可用于翻看日历
        // 设置日历为中文模式
        ips200pro_calendar_display(2025, 2, IPS200PRO_CALENDAR_CHINESE);
