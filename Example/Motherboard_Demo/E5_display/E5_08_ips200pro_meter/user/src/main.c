...
// *************************** 例程测试说明 ***************************
// 1.该例程为IPS200PRO显示仪表控件控制示例
// 2.核心板烧录本例程 插在主板上
//   2寸IPSPRO 显示模块插在主板的屏幕接口排座上
//   请注意引脚对应 不要插错
// 2.电池供电 上电后 2寸IPS 屏幕亮起 并且在屏幕底部显示“Meter测试”
// 3.主页面将显示两种不同的仪表控件，并且演示指向不同内容的效果
...
int main(void)
{
    ...
    // 初始化屏幕并创建一个页面
    page_id[0] = ips200pro_init("Meter测试", IPS200PRO_TITLE_BOTTOM, 30);
    ...
    // 创建一个仪表，指示角度的类型
    meter_id[0] = ips200pro_meter_create((240 - 120) / 2, 0, 120, METER_ANGLE);

    // 设置仪表各个颜色属性
    ips200pro_set_color(meter_id[0], COLOR_FOREGROUND, IPS200PRO_RGB888_TO_RGB565(0x43, 0x4A, 0x61));
    ips200pro_set_color(meter_id[0], COLOR_BACKGROUND, RGB565_WHITE);
    ips200pro_set_color(meter_id[0], COLOR_BORDER, RGB565_BLACK);
    ips200pro_set_color(meter_id[0], COLOR_MRTER_INDICATOR, RGB565_BLACK);
    ips200pro_set_color(meter_id[0], COLOR_MRTER_TICKS, RGB565_BLACK);
    // 设置仪表的数值
    ips200pro_meter_set_value(meter_id[0], 150);

    // 创建一个仪表，用于指示速度或数值
    meter_id[1] = ips200pro_meter_create((240 - 150) / 2, 120, 150, METER_SPEED);
    ips200pro_meter_set_value(meter_id[1], 25);

    // 设置仪表各个颜色属性
    ips200pro_set_color(meter_id[1], COLOR_FOREGROUND, RGB565_BLACK);
    ips200pro_set_color(meter_id[1], COLOR_BACKGROUND, RGB565_WHITE);
    ips200pro_set_color(meter_id[1], COLOR_BORDER, IPS200PRO_RGB888_TO_RGB565(0xC5, 0xD5, 0xEC));
    ips200pro_set_color(meter_id[1], COLOR_MRTER_INDICATOR, RGB565_RED);
    ips200pro_set_color(meter_id[1], COLOR_MRTER_TICKS, RGB565_BLACK);
    ...
    while(true)
    {
        ...
        // 设置仪表的数值
        ips200pro_meter_set_value(meter_id[0], meter_value[0]);
        ips200pro_meter_set_value(meter_id[1], meter_value[1]);
        system_delay_ms(20);
