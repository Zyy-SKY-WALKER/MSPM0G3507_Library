...
// *************************** 例程测试说明 ***************************
// 1.该例程为IPS200PRO显示时钟控件控制示例
// 2.核心板烧录本例程 插在主板上
//   2寸IPSPRO 显示模块插在主板的屏幕接口排座上
//   请注意引脚对应 不要插错
// 2.电池供电 上电后 2寸IPS 屏幕亮起 并且底部显示“Clock测试”
// 3.主页面将显示一个时钟控件，并且不需要代码控制，自动走时
...
int main(void)
{
    ...
    // 初始化屏幕并创建一个页面
    page_id[0] = ips200pro_init("Clock测试", IPS200PRO_TITLE_BOTTOM, 30);
    ...
    clock_id = ips200pro_clock_create(20, 0, 200, CLOCK_ANALOG);

    // 设置时钟的各种颜色属性
    ips200pro_set_color(clock_id, COLOR_FOREGROUND, IPS200PRO_RGB888_TO_RGB565(0x5F, 0x5E, 0x5F));
    ips200pro_set_color(clock_id, COLOR_BACKGROUND, RGB565_WHITE);
    ips200pro_set_color(clock_id, COLOR_BORDER, RGB565_BLACK);//IPS200PRO_RGB888_TO_RGB565(129, 120, 126)
    ips200pro_set_color(clock_id, COLOR_CLOCK_HOUR, RGB565_BLACK);
    ips200pro_set_color(clock_id, COLOR_CLOCK_MINUTE, RGB565_BLACK);
    ips200pro_set_color(clock_id, COLOR_CLOCK_SECOND, RGB565_RED);
    ips200pro_set_color(clock_id, COLOR_CLOCK_TICKS, RGB565_BLACK);

    // 设置系统时间
    ips200pro_set_time(15, 30, 00);
