...
// *************************** 例程测试说明 ***************************
// 1.该例程为IPS200PRO显示示波器控件控制示例
// 2.核心板烧录本例程 插在主板上
//   2寸IPSPRO 显示模块插在主板的屏幕接口排座上
//   请注意引脚对应 不要插错
// 2.电池供电 上电后 2寸IPS 屏幕亮起 并且底部显示“Waveform测试”
// 3.主页面将显示一个示波器控件，并且展示示波器控件的不同展示效果
...
int main(void)
{
    ...
    // 初始化屏幕并创建一个页面
    page_id[0] = ips200pro_init("Waveform测试", IPS200PRO_TITLE_BOTTOM, 30);
    ...
    // 创建一个显示波形的控件,每条波形最多显示200(宽度)个数据，数据范围0-149(高度 - 1)
    waveform_id = ips200pro_waveform_create(20, 20, 200, 150);

    // 设置波形控件的背景颜色
    ips200pro_set_color(waveform_id, COLOR_BACKGROUND, IPS200PRO_RGB888_TO_RGB565(0xE0, 0xE0, 0xE0));
    ...
    while(true)
    {
        ...
        // 绘制一条曲线,每次发送一个点的数据，通常用于将实时数据绘制绘制成波形图
        for(i = 0; 200 > i; i++)
        {
            value_temp = (uint16)(74 * sin((float)i * 5 * 3.141592 / 180) + 75);
            ips200pro_waveform_add_value(waveform_id, 1, &value_temp, 1, RGB565_BLUE);
            system_delay_ms(20);
            if(50 == i)
            {
                system_delay_ms(20);
        ...
        // 增加一条曲线，一次性设置曲线的所有数据，一个控件最多显示5条曲线
        for(i = 0; 200 > i; i++)
        {
            line_value[i] = (uint16)(74 * sin((float)(i + 180) * 5 * 3.141592 / 180) + 75);
        }
        // 一次性将曲线数据发送给屏幕，通常用于历史数据绘制成波形图，或者显示CCD图像等
        ips200pro_waveform_add_value(waveform_id, 2, line_value, 200, RGB565_BROWN);

        // 波形完成绘制，暂停观察
        system_delay_ms(1000);

        // 将曲线设置为连线
        ips200pro_waveform_line_type(waveform_id, 1);

        // 波形完成绘制，暂停观察
        system_delay_ms(1000);

        // 将曲线设置为散点图
        ips200pro_waveform_line_type(waveform_id, 0);

        // 波形完成绘制，暂停观察
        system_delay_ms(1000);

        // 将所有的波形清除
        ips200pro_waveform_clear(waveform_id);
