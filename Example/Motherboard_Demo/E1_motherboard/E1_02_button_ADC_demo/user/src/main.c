...
// 2.主板与核心板连接好，使用电池供电，将调试下载器或者 USB-TTL 模块连接电脑
...
//      Battery valtage is : xxxx. (0-4096)
//      Battery valtage is : xx.xxx. (0V-12V)
// 5.示数会根据电池电压变化
// 如果发现现象与说明严重不符 请参照本文件最下方 例程常见问题说明 进行排查

// **************************** 代码区域 ****************************

#define VOLTAGE_PORT    ADC0_CH7_A22 // 定义检测端口
...
int main(void)
{
    ...
    while(true)
    {
        ...
        valtage_now = 36.3 * valtage_adc / 4096;                                // 根据逐飞科技学习主板分压计算 测得应该是电池电压 1/4 那么最大量程 36.3V
        printf("\r\nBattery Valtage is : %d. (0-4096)", valtage_adc);           // 由于电阻会有误差 ADC检测本身也会有一些误差 所以这里对应数值可能跟实际电压有较多误差
        printf("\r\nBattery Valtage is : %2.3f. (0V-12V)", valtage_now);        // 这里换算的一定会跟实际电池电压有误差 需要按照实际的检测结果为准 可以自行矫正
