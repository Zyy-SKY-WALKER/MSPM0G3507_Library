...
//
// 6.打开逐飞助手，设置为UDP，选择合适的本机地址后点击连接
...
//
// 6.当模块初始化完成后会通过 DEBUG 串口输出当前模块的主要信息：固件版本、IP信息、MAC信息
//
// 7.本例程是 UDP 例程
//   目标的 IP 与端口默认使用 zf_device_wifi_spi.h 中 WIFI_SPI_TARGET_IP 与 WIFI_SPI_TARGET_PORT 定义
//   实际测试需要根据现场 远端 的实际 IP 地址与端口设置
//
// 8.当本机设备成功连接到目标后（例如电脑使用逐飞助手上位机进入UDP模式 然后本机连接到电脑的 IP 与端口）
//   本例程会先通过调试串口发送链接信息，然后向 目标 发送一段测试数据， 之后可以使用上位机发送数据给设备，设备收到之后会将数据发回给上位机
//
